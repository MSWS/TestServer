...
public final class UpdateChecker {
		...
		return CompletableFuture.supplyAsync(() -> {
			...
			try {
				...
				if (!element.isJsonArray()) {
					return new UpdateResult(UpdateReason.INVALID_JSON, "Unknown");
				}
				...
				if (latest == null) {
					return new UpdateResult(UpdateReason.UNSUPPORTED_VERSION_SCHEME, newest);
				} else if (latest.equals(current)) {
					return new UpdateResult(
							current.equals(newest) ? UpdateReason.UP_TO_DATE : UpdateReason.UNRELEASED_VERSION, newest);
				} else if (latest.equals(newest)) {
					return new UpdateResult(UpdateReason.NEW_UPDATE, latest, newest);
				}
			} catch (IOException e) {
				return new UpdateResult(UpdateReason.COULD_NOT_CONNECT, "Unknown");
	...
	public final class UpdateResult {
		...
		private UpdateResult(UpdateReason reason, String newestVersion, String onlineVersion) {
			this.reason = reason;
			this.newestVersion = newestVersion;
			this.onlineVersion = onlineVersion;
		}

		private UpdateResult(UpdateReason reason, String onlineVersion) {
